    ...
    <svg :x="graphRect.x" :y="graphRect.y" :width="graphRect.w" :height="graphRect.h">
      <g id="svggraph-rings-planet">
        ...
        <path
          ...
          opacity="37.5%"
          stroke-dasharray="2"
          marker-mid="url(#svggraph-rings-grad)"
          marker-end="url(#svggraph-rings-grad)"
        />
      ...
      <g id="svggraph-biomes-data">
        <g v-for="(r, i) of ringData" :key="r.id">
          ...
          />
          <line
            :x1="(r.center * 460.0) / 5.0"
            y1="90"
            :x2="(r.center * 460.0) / 5.0"
            y2="58"
            stroke="white"
            stroke-width="1"
          />
...
onMounted(() => {
  props.rings
    .toSorted((a, b) => a.innerRadius - b.innerRadius)
    .forEach((r) =>
      ringData.value.push({
        id: r.id,
