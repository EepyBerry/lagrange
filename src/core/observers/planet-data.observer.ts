...
export class PlanetDataObserver extends Observer {
  ...
  private registerLightingDataUpdates(data: PlanetData, sunLight: DirectionalLight, ambLight: AmbientLight, atmosphere: AtmosphereMeshData, lensFlare: LensFlareEffect): void {
    ...
    this.registerEventHandler('sunLightColor', genericHandler(() => {
      sunLight.color.set(data.sunLightColor);
      lensFlare.uniforms.colorGain.value = data.sunLightColor;
    }));
    this.registerEventHandler('sunLightIntensity', genericHandler(() => sunLight.intensity = data.sunLightIntensity));
    this.registerEventHandler('ambLightColor',     genericHandler(() => ambLight.color.set(data.ambLightColor)));
    this.registerEventHandler('ambLightIntensity', genericHandler(() => ambLight.intensity = data.ambLightIntensity));
  }
  ...
  private registerPlanetRenderingDataUpdates(
    ...
    this.registerEventHandler('planetRadius', genericHandler(() => {
    ...
    this.registerEventHandler('planetAxialTilt', genericHandler(() => {
    ...
    this.registerEventHandler('planetRotation', genericHandler(() => {
    ...
    this.registerEventHandler('planetWaterLevel',              genericHandler(() => (planet.uniforms!.pbr.waterLevel.value = data.planetWaterLevel)));
    this.registerEventHandler('planetWaterRoughness',          genericHandler(() => (planet.uniforms!.pbr.metallicRoughness.value.x = data.planetWaterRoughness)));
  ...
  private registerSurfaceDataUpdates(data: PlanetData, planet: PlanetMeshData): void {
    this.registerEventHandler('planetSurfaceShowBumps',    genericHandler(() => (planet.uniforms!.flags.array[2] = +data.planetSurfaceShowBumps)));
    this.registerEventHandler('planetSurfaceBumpStrength', genericHandler(() => (planet.uniforms!.bumpStrength.value = data.planetSurfaceBumpStrength)));
    // Color
    this.registerEventHandler('planetSurfaceColorRamp', genericHandler(() => {
    ...
    // Warping
    this.registerEventHandler('planetSurfaceShowWarping', genericHandler(() => (planet.uniforms!.flags.array[0] = +data.planetSurfaceShowWarping)));
    this.registerEventHandler('planetSurfaceNoise._warpFactor', genericHandler(() => {
    ...
    // Displacement
    this.registerEventHandler('planetSurfaceShowDisplacement',         genericHandler(() => (planet.uniforms!.flags.array[1] = +data.planetSurfaceShowDisplacement)));
    this.registerEventHandler('planetSurfaceDisplacement._factor',     genericHandler(() => (planet.uniforms!.surface.displacement.params.value.x = data.planetSurfaceDisplacement.factor)));
    this.registerEventHandler('planetSurfaceDisplacement._epsilon',    genericHandler(() => (planet.uniforms!.surface.displacement.params.value.y = data.planetSurfaceDisplacement.epsilon)));
    this.registerEventHandler('planetSurfaceDisplacement._multiplier', genericHandler(() => (planet.uniforms!.surface.displacement.params.value.z = data.planetSurfaceDisplacement.multiplier)));
    this.registerEventHandler('planetSurfaceDisplacement._frequency',  genericHandler(() => (planet.uniforms!.surface.displacement.noise.value.x = data.planetSurfaceDisplacement.frequency)));
    this.registerEventHandler('planetSurfaceDisplacement._amplitude',  genericHandler(() => (planet.uniforms!.surface.displacement.noise.value.y = data.planetSurfaceDisplacement.amplitude)));
    this.registerEventHandler('planetSurfaceDisplacement._lacunarity', genericHandler(() => (planet.uniforms!.surface.displacement.noise.value.z = data.planetSurfaceDisplacement.lacunarity)));
    this.registerEventHandler('planetSurfaceDisplacement._octaves',    genericHandler(() => (planet.uniforms!.surface.displacement.noise.value.w = data.planetSurfaceDisplacement.octaves)));

    // Noise
    this.registerEventHandler('planetSurfaceNoise._layers',     genericHandler(() => (planet.uniforms!.surface.warping.value.x = data.planetSurfaceNoise.layers)));
    this.registerEventHandler('planetSurfaceNoise._frequency',  genericHandler(() => (planet.uniforms!.surface.noise.value.x = data.planetSurfaceNoise.frequency)));
    this.registerEventHandler('planetSurfaceNoise._amplitude',  genericHandler(() => (planet.uniforms!.surface.noise.value.y = data.planetSurfaceNoise.amplitude)));
    this.registerEventHandler('planetSurfaceNoise._lacunarity', genericHandler(() => (planet.uniforms!.surface.noise.value.z = data.planetSurfaceNoise.lacunarity)));
    this.registerEventHandler('planetSurfaceNoise._octaves',    genericHandler(() => (planet.uniforms!.surface.noise.value.w = data.planetSurfaceNoise.octaves)));
  }
  ...
  private registerBiomeDataUpdates(data: PlanetData, planet: PlanetMeshData): void {
    ...
    this.registerEventHandler('biomesParams[element]', keyedHandler((event) => {
  ...
  private registerCloudDataUpdates(data: PlanetData, clouds: CloudsMeshData): void {
    ...
    this.registerEventHandler('cloudsRotation', genericHandler(() => {
    ...
    // Warping
    this.registerEventHandler('cloudsShowWarping',       genericHandler(() => (clouds.uniforms!.flags.array[0] = +data.cloudsShowWarping)));
    this.registerEventHandler('cloudsNoise._warpFactor', genericHandler(() => {
      // noinspection JSSuspiciousNameCombination
    ...
    // Displacement
    this.registerEventHandler('cloudsShowDisplacement',         genericHandler(() => (clouds.uniforms!.flags.array[1] = +data.cloudsShowDisplacement)));
    this.registerEventHandler('cloudsDisplacement._factor',     genericHandler(() => (clouds.uniforms!.displacement.params.value.x = data.cloudsDisplacement.factor)));
    this.registerEventHandler('cloudsDisplacement._epsilon',    genericHandler(() => (clouds.uniforms!.displacement.params.value.y = data.cloudsDisplacement.epsilon)));
    this.registerEventHandler('cloudsDisplacement._multiplier', genericHandler(() => (clouds.uniforms!.displacement.params.value.z = data.cloudsDisplacement.multiplier)));
    this.registerEventHandler('cloudsDisplacement._frequency',  genericHandler(() => (clouds.uniforms!.displacement.noise.value.x = data.cloudsDisplacement.frequency)));
    this.registerEventHandler('cloudsDisplacement._amplitude',  genericHandler(() => (clouds.uniforms!.displacement.noise.value.y = data.cloudsDisplacement.amplitude)));
    this.registerEventHandler('cloudsDisplacement._lacunarity', genericHandler(() => (clouds.uniforms!.displacement.noise.value.z = data.cloudsDisplacement.lacunarity)));
    this.registerEventHandler('cloudsDisplacement._octaves',    genericHandler(() => (clouds.uniforms!.displacement.noise.value.w = data.cloudsDisplacement.octaves)));
    // Noise
    this.registerEventHandler('cloudsNoise._frequency',  genericHandler(() => (clouds.uniforms!.noise.value.x = data.cloudsNoise.frequency)));
    this.registerEventHandler('cloudsNoise._amplitude',  genericHandler(() => (clouds.uniforms!.noise.value.y = data.cloudsNoise.amplitude)));
    this.registerEventHandler('cloudsNoise._lacunarity', genericHandler(() => (clouds.uniforms!.noise.value.z = data.cloudsNoise.lacunarity)));
    this.registerEventHandler('cloudsNoise._octaves',    genericHandler(() => (clouds.uniforms!.noise.value.w = data.cloudsNoise.octaves)));
    // Color
    this.registerEventHandler('cloudsColor',             genericHandler(() =>  (clouds.uniforms!.color.value = data.cloudsColor)));
    this.registerEventHandler('cloudsColorRamp',         genericHandler(() =>  {
  ...
  private registerAtmosphereDataUpdates(data: PlanetData, atmosphere: AtmosphereMeshData): void {
    this.registerEventHandler('atmosphereEnabled', genericHandler(() => (atmosphere.mesh!.visible = data.atmosphereEnabled)));
    this.registerEventHandler('atmosphereHeight',  genericHandler(() => {
  ...
  private registerRingsDataUpdates(data: PlanetData, ringAnchor: Group, ringsMeshData: RingMeshData[]): void {
    this.registerEventHandler('ringsEnabled', genericHandler(() => {
      ringAnchor.visible = data.ringsEnabled
      ringAnchor.children.forEach(r => r.visible = data.ringsEnabled)
    }));
    this.registerEventHandler('ringsParams', globalHandler(() => {
    ...
    this.registerEventHandler('ringsParams[element]', keyedHandler((event) => {
