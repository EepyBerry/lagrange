...
type RawShaderData = { default: string }

const IMPORT_REGEXP = /^@import (.*);$/gm

const GLSL_CORE_FILES: Record<string, RawShaderData> = import.meta.glob('@assets/glsl/core/*.glsl', {
  eager: true,
  query: '?raw',
})
const GLSL_BAKING_FILES: Record<string, RawShaderData> = import.meta.glob('@assets/glsl/baking/*.glsl', {
  eager: true,
  query: '?raw',
})
const GLSL_FUNCTION_FILES: Record<string, RawShaderData> = import.meta.glob('@assets/glsl/functions/*.func.glsl', {
  eager: true,
  query: '?raw',
})
...
export function fetch(filename: string, type: ShaderFileType): string {
  let glslUris: string[]
  let glslRecords: Record<string, RawShaderData>
